...
nvertp=ep.data(TRAN_FILE,'NVERTP')
print("nverts array:", nvertp.data[0:nvertp.nPts])
print(f"Number of vertices: {nvertp.nPts}")

profile=ep.data(TRAN_FILE, PROFILE)
print(profile)
profile=profile.data[0:profile.nPts]
...
# ============================================================================
# Map centroids to mesh points (rmesh, zmesh) and extract temperature values
# ============================================================================
...
# Find nearest mesh point for each centroid and extract temperature values
distances, indices = tree.query(centroids)
values = profile[indices]

print(f"Temperature values mapping:")
print(f"  Number of centroids: {len(centroids)}")
print(f"  Number of mesh points (rmesh): {len(rmesh)}")
print(f"  Temperature values extracted: {len(values)}")
print(f"  Temperature range: {values.min():.2f} - {values.max():.2f}")
print(f"  Max distance to nearest mesh point: {distances.max():.2e}")

print(f"\nFirst 10 centroids and their temperatures:")
print(f"{'Quad':>6}  {'R cent':>10}  {'Z cent':>10}  {'R mesh':>10}  {'Z mesh':>10}  {'Te (eV)':>12}  {'Dist':>10}")
print("-" * 72)
...
print(f"\n✓ Successfully created values array for algorithm.py")
print(f"{'='*70}\n")

fig, ax = plt.subplots(1, 1, figsize=(14, 10))

# Create normalization and colormap for temperature values
norm = plt.Normalize(vmin=values.min(), vmax=values.max())
cmap = plt.cm.viridis

# Draw all quads with temperature colors
patches = []
colors = []
for qi, quad in enumerate(quads):
    poly_nodes = nodes[quad]
    poly = Polygon(poly_nodes, closed=True)
    patches.append(poly)
    colors.append(values[qi])

pc = PatchCollection(patches, cmap=cmap, norm=norm, edgecolors='black', linewidths=0.5, alpha=0.9)
pc.set_array(np.array(colors))
ax.add_collection(pc)

# Add colorbar
cbar = plt.colorbar(pc, ax=ax, label='Temperature (eV)')

# Draw vertices
ax.scatter(nodes[:, 0], nodes[:, 1], c='red', s=5, marker='.', label='vertices', zorder=5, alpha=0.5)

# Draw centroids
centroid_indices = np.arange(4, len(rvertp), 5)
centroids_r = rvertp[centroid_indices]
centroids_z = zvertp[centroid_indices]
ax.scatter(centroids_r, centroids_z, c='darkred', s=20, marker='x', label='centroids', zorder=6, linewidths=1)

# Set axis limits
ax.set_xlim(nodes[:, 0].min() - 0.01, nodes[:, 0].max() + 0.01)
ax.set_ylim(nodes[:, 1].min() - 0.01, nodes[:, 1].max() + 0.01)

ax.set_aspect('equal')
ax.set_xlabel('R (m)')
ax.set_ylabel('Z (m)')
ax.set_title(f'Temperature map: {n_quads} quads | Te range: {values.min():.2f} - {values.max():.2f} eV')
...
# ============================================================================
# LINE OF SIGHT INTERSECTION ALGORITHM
# ============================================================================
...
# ============================================================================
# Apply line of sight to extract temperature profile
# ============================================================================

# Define line of sight endpoints
point1 = np.array([4.5, 0.0])
point2 = np.array([3.0, 2.0])
...
print(f"\n{'t_enter':>10}  {'t_exit':>10}  {'Δt':>10}  {'Te (eV)':>10}")
print("-" * 42)
...
ax = axes[0]
ax.set_title("Temperature map + Line of Sight", fontsize=12, fontweight='bold')
ax.set_aspect("equal")
...
pc.set_array(np.array(colors))
ax.add_collection(pc)
plt.colorbar(pc, ax=ax, label="Temperature (eV)")
...
# Mark line endpoints
ax.plot(*point1, "go", markersize=12, label="Point 1 (4.5, 0)", zorder=11, markeredgecolor='darkgreen', markeredgewidth=2)
ax.plot(*point2, "mo", markersize=12, label="Point 2 (3, 2)", zorder=11, markeredgecolor='purple', markeredgewidth=2)
...
# --- Right: 1D temperature profile ---
ax2 = axes[1]
ax2.set_title("1D Temperature Profile Along Line of Sight", fontsize=12, fontweight='bold')
...
sm.set_array([])
plt.colorbar(sm, ax=ax2, label="Temperature (eV)")

plt.tight_layout()
plt.show()

print(f"\n✓ Line of Sight analysis complete")
print(f"{'='*70}\n")
